# Firebase Cloud Messaging for AL
...
for user in users:
    ...
    "WHERE accuracies.activity_type_id != (%s) AND accuracies.user_id = (%s) AND accuracies.accuracy_type = 'proposed') AS total " + 
    ...
    if len(results) > 0 and uploaded_labels[0][0] > 0:
        ...
        for i , row in df.iterrows():
            temp += str(row.activity_type) + "= " + str(row.value)+"\n"
            token = row.firebase_token
        
            uploaded_labels = "%s labels" % (row.uploaded_labels)
            user_created_at = json_serial(row.user_created_at)
            
            if(i == 0):
                now = datetime.now()            
                diff = now - row.user_created_at
